...
//        }
    ...
    private void debugBufferedViews() {
        if (MLog.ENABLE_DEBUG) {
            MLog.d("bufferedViews: %s; buffer index %d, adapter index %d", bufferedViews, bufferIndex,
                    adapterIndex);
        }
    }
    ...
    void flippedToView(final int indexInAdapter, boolean isPost) {
        ...
        if (indexInAdapter >= 0 && indexInAdapter < adapterDataCount) {
            ...
            if (indexInAdapter == adapterIndex + 1) { //forward one page
                ...
                MLog.e("Should not happen: indexInAdapter %d, adapterIndex %d", indexInAdapter,
                        adapterIndex);
            }
        } else {
